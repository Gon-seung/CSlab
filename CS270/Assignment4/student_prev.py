...
# state: x position, y position, left sensor, front sensor, right sensor, orientation
# number of elements of state: 6
# action: move forward, turn left, turn right
same = 0
def reward(prev_coor, coor, done, steps, visited_val):
    """
    ...
    if coor == goal:
        ...
        return -500
    elif same>=3 or visited_val==4:
        return -50
    # elif steps<150 and coor==(5, 6) or coor==(6, 5) and prev_coor!=coor:
    #     return 20
    elif steps>=3 and coor==(1, 1):
        return -100
...
def test_network():
    ...
    set_speed(3)
    test()
    (x, y), ori, sensor, done = reset_map(s)
